...
namespace Monit95App.Services.OneTwoThree.QuestionProtocol
{
    public class QuestionProtocolService : IQuestionProtocolService
    {
        ...
        public void EditQuestionMarks(int participTestId, string schoolId, QuestionProtocolDto protocolDto)
        {
            ...
            (participTest.Grade5, participTest.GradeString) = GetGrades(countOfPassedGeneralQuestions, percentOfNonGeneralQuestionMarks, _marksCountsByProjectId[participTest.ProjectTest.Test.NumberCode]);
            // TODO: Solve Grade5!
        ...
        public IEnumerable<QuestionListDto> GetQuestionListDtos(string schoolId, int projectTestId)
        {
            var participTests = context.ParticipTests
                .AsNoTracking()
                .Where(p => p.Particip.SchoolId == schoolId && p.ProjectTestId == projectTestId && p.ProjectTest.IsOpen)
        ...
        private readonly Dictionary<string, GradesOptions> _marksCountsByProjectId = new Dictionary<string, GradesOptions>
        {
            ["0101"] = new GradesOptions
            {
                MinCountForThree = 7,
                MaxCountForThree = 8,
                MinCountForFour = 9,
