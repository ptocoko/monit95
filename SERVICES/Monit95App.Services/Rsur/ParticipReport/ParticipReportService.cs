...
namespace Monit95App.Services.Rsur.ParticipReport
{
    public class ParticipReportService : IParticipReportService
    {
        ...
        public ServiceResult<ParticipExtendReport> GetExtendReport(int rsurParticipTestId, int? areaCode = null, string schoolId = null)
        {
            ...
            serviceResult.Result.Grade100 = serviceResult.Result.WithSkills ? entity.Grade100 : null;
            ...
            if (serviceResult.Result.WithSkills)
            {
                ...
                var questions = context.RsurElementResults
                    ...
                    .Select(s => new
                    {
                        ...
                        RsurQuestionNumbers = context.RsurQuestions.Where(p => p.EgeQuestionId == s.ElementId && p.TestId == testId).Select(s1 => s1.Order).Single().ToString()
                    });
                ...
                serviceResult.Result.SkillQuestionResults = groupedQuestion.Select(s => new SkillQuestionResult
                {
                    ...
                    QuestionResults = s.Select(q => new EgeQuestionResult
                    {
                        ...
                        RsurQuestionNumbers = q.RsurQuestionNumbers
                    })
                    .OrderBy(ob => int.Parse(ob.RsurQuestionNumbers))
                    .ToList()
                })
                .OrderBy(ob => ob.QuestionResults.Select(s => int.Parse(s.RsurQuestionNumbers)).First())
                .ToList();
            } else {
        ...
        private IEnumerable<TestDto> GetTestNames(IQueryable<RsurTestResult> entities)
        {
            var tests = entities.Select(s => s.RsurParticipTest.RsurTest.Test).Distinct();
            var orderedTests = tests.OrderBy(ob => ob.NumberCode);
            return orderedTests.AsEnumerable().Select(s => {
                if (s.IsRsurTestWithSkills.HasValue && s.IsRsurTestWithSkills.Value)
                {
                    return new TestDto { Code = s.NumberCode.Trim(), Name = $"{s.Name.Trim()}, методическая часть" };
                } 
                else
                {
                    return new TestDto { Code = s.NumberCode.Trim(), Name = $"{s.NumberCode} - {s.Name.Trim()}" };
                }
            }).Distinct(new TestDtoComparer());
        }
