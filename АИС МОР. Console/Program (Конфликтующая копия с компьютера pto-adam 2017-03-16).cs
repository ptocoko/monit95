...
namespace Monit95App.ConsoleApp
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var currentTestGuid = new Guid("5CC897F0-55FD-409C-9D98-15EE88511862"); //
            ...
            var participGroupResults = testResultService.GetParticipGroupResultByDate(currentProjectCode, currentTestGuid, currentTestDate).ToList();
            int countProcessedResults = 0;            
            foreach (var result in participGroupResults)
            {
                sheet = excerInitBook.Sheets["data"];                
                //Заполняем реквизиты
                var resultTest1 = result.First();                               
                ...
                int index = 0;
                foreach (var r in result.OrderBy(x => x.TestNumber))
                {
                ...
                try
                {
                    sheet = excerInitBook.Sheets[startSheetName + result.Count()];
                }
                catch(COMException)
                {
                    Console.WriteLine(startSheetName + result.Count());
                }
