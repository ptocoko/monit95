...
namespace ProtocolGenerator
{
    public class ITakeEge
    {
        ...
        public void GenerateRaspr(int projectId, string path, string templateName, bool isArchieving)
        {
            var particips = context.Particips.Where(pt => pt.ProjectId == projectId)
                .Include("School.Area")
                .ToList()
                .Select(pt => new {
                    Fio = $"{pt.Surname} {pt.Name} {pt.SecondName}",
                    pt.DocumNumber,
                    pt.SchoolId,
                    SchoolName = pt.School.Name,
                    AreaName = pt.School.Area.Name
                })
                .OrderBy(ob => ob.Fio)
                .GroupBy(gb => new { gb.SchoolId, gb.SchoolName, gb.AreaName });
