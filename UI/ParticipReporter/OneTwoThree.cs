...
namespace ParticipReporter
{
    public class OneTwoThree
    {
        ...
        public void GenerateCards()
        {
            ...
            string destFolder = $@"\\192.168.88.223\файлы_пто\Работы\[2016-77] - 1-3 классы\2021\карты";
            var schoolIds = context.ParticipTests.Where(pt => pt.ProjectTest.ProjectId == 36)
                .Select(pt => pt.Particip.SchoolId)
                .Distinct()
                .ToList()
                .Where(schoolId => !Directory.GetDirectories(destFolder).Select(d => d.Split('\\').Last()).Contains(schoolId))
                .ToList();
            var projectTests = context.ProjectTests.Where(pj => pj.ProjectId == 36)
