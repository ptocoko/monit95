...
export class ReportListComponent implements AfterViewInit, OnDestroy {
	...
	private createRequest(): Observable<ReportsList> {
		...
		const testCode = getTestCode(this.selectedTest);
...
function getTestCode(selectedTestCode: string): string {
	return selectedTestCode && selectedTestCode !== TESTNAME_DEFAULT_SELECTION ? selectedTestCode : null;
}