...
export class MarksProtocolComponent implements AfterViewInit, OnDestroy {
	...
	ngAfterViewInit(): void {
		...
		const labels = document.querySelectorAll<HTMLLabelElement>('.markLabel');
		const maxWidth = Array.from(labels).map(labelElem => labelElem.clientWidth).reduce((agg, curr) => agg = agg < curr ? curr : agg, 0);
		labels.forEach(labelEl => labelEl.style.width = maxWidth + 'px');
	}
