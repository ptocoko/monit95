...
namespace AccountsCreator
{
    class Program
    {
        ...
        private static void CreateSchool(School school, string password)
        {
            var context = new CokoContext();
            context.Monit95Users.Add(new Monit95User { Login = school.Id, Password = password });
            //context.SaveChanges();
            context.Schools.Add(school);
            context.SaveChanges();

            accountsManager.CreateUser(school.Id, password);
            accountsManager.AddRoleToUser(school.Id, "school");
        }
    }
}
